...
def create_app(
    ...
    @app.post("/sessions/{session_id}/rules/yaml")
    def load_yaml_rules(session_id: str, data: YamlRuleRequest) -> list[DetectionRule]:
        try:
            from tools.sigma_yaml import load_sigma_yaml

            loaded = load_sigma_yaml(data.path)
            created: list[DetectionRule] = []
            for r in loaded:
                rule = mediator.add_rule(
                    session_id,
                    RuleCreate(
                        name=r["name"],
                        severity=r.get("severity", "medium"),
                        technique_ids=[r["technique_id"]] if r.get("technique_id") else [],
                        patterns={"stdout": [r["pattern_stdout"]]} if r.get("pattern_stdout") else {},
                    ),
                )
                created.append(rule)
            return created
    ...
    try:
        app.mount("/dashboard", StaticFiles(directory="api/static", html=True), name="dashboard")
    except Exception:
        pass
